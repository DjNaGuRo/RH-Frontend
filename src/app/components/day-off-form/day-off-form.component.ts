...
export class DayOffFormComponent {
  ...
  ngOnInit(): void {
    ...
    this.dayOffForm
      .get('startDate')
      ?.valueChanges.subscribe((startDate) => (this.minEndDate = startDate));
    this.dayOffForm
      .get('endDate')
      ?.valueChanges.subscribe((endDate) => (this.maxStartDate = endDate));

    this.userService.getCollaborator().subscribe((collaborator) => {
      this.collaborator = collaborator;
          this.selectedValue = 'CP';
  ...
  get administrator() {
    if (
      this.collaborator?.role.toString() ===
      CollaboratorRoleEnum[CollaboratorRoleEnum.ADMINISTRATOR]
    ) {
      return true;
    } else {
      return false;
  ...
  get DayOffTypeIsValid() {
    return this.dayOffForm.get('dayOffType')?.valid;
  }

  get DayOffTypeIsInvalid() {
    return this.dayOffForm.get('dayOffType')?.invalid;
  }

  get StartDateIsValid() {
    return this.dayOffForm.get('startDate')?.valid;
  }

  get StartDateIsInvalid() {
    return this.dayOffForm.get('startDate')?.invalid;
  }
  ...
  sendForm($event: any) {
    ...
    this.dayOffService.createDayOff(dayOffToCreate).subscribe(
      () => {
        this.modalService.dismissAll();
  ...
  formateDayOff(): DayOffToCreate {
    let dayOffTypeToCreate: DayOffTypeEnum =
      this.dayOffForm.get('dayOffType')?.value;

    let startDateFormValue = this.dayOffForm.get('startDate')?.value;
    let startDateFormat = new Date(
      startDateFormValue.year,
      startDateFormValue.month - 1,
      startDateFormValue.day
    );
    ...
    let endDateFormat = new Date(
      endDateFormValue.year,
      endDateFormValue.month - 1,
      endDateFormValue.day
    );
